...
func AtualizarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	if publicacaoSalvaNoBanco.AutorID != usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("Não é possível alterar uma publicação que não seja sua"))
		return
	}
...
func DeletarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	publicacoes := repositorios.NovoRepositorioDePublicacoes(db)

	publicacaoSalvaNoBanco, erro := publicacoes.BuscarPorId(publicacaoID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
	...
	if erro = publicacoes.Deletar(publicacaoID); erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
